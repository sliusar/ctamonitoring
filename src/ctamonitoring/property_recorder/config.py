...
import traceback
from ACS import NoSuchCharacteristic  # @UnresolvedImport This is generated by ACS and present in any ACS setup
...
class PropertyAttributeHandler(object):
    ...
    def _getCdbEntry(attribute, acs_property):
        ...
        try :
            ...
            print 'The attrib:  '+ str(attribute.name) + ' does not exist'
            return None
    ...
    def _process_attribute(attribute, raw_value):
        try:
            #print 'decoding ' + attribute.name #TODO: all this should be done withe logger
            #print 'raw value is: ' + str(raw_value)
            value = AttributeDecoder.decode_attribute(raw_value, attribute.decoding)
        except Exception, e:
            traceback.print_exc() #TODO use better the exception, do not print in the console
            value = None 
        
        #Check those cases when it has to be positive
        if (attribute.isPositive) and (value is not None) and (value < 0.0):
            print 'value is not valid, ignoring entry' #TODO use better the exception, do not print in the console
            value = None
